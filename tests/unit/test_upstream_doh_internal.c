...
static void test_doh_post_header_failures(void **state) {
    ...
    assert_int_equal(doh_post_with_handle(NULL, (CURL *)0x11, &server, 0, 0, 100, query, sizeof(query), &resp, &resp_len, NULL), -1);
...
static void test_doh_post_transport_and_status_failures(void **state) {
    ...
    assert_int_equal(doh_post_with_handle(&client, (CURL *)0x11, &server, 0, 0, 100, query, sizeof(query), &resp, &resp_len, NULL), -1);
...
static void test_doh_post_success_and_http_version_counters(void **state) {
    ...
    assert_int_equal(doh_post_with_handle(&client, (CURL *)0x12, &server, 0, 0, 100, query, sizeof(query), &resp, &resp_len, NULL), 0);
    free(resp);
...
static void test_doh_post_http_version_preference(void **state) {
    ...
    assert_int_equal(doh_post_with_handle(NULL, (CURL *)0x11, &server, 0, 0, 100, query, sizeof(query), &resp, &resp_len, NULL), 0);
    free(resp);
